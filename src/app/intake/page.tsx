...
export default function IntakePage() {
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[var(--bg-primary)] px-6 text-center">
        <div className="w-full max-w-[520px]">
          ...
          <p className="mt-2 text-sm" style={{ color: 'var(--text-muted)' }}>Scorecard, Founder DNA, failure probability, war plan.</p>
          <div className="mt-8 h-2 overflow-hidden rounded-full" style={{ background: 'var(--border)' }}>
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[var(--bg-primary)] px-6 py-20">
      <motion.form
        onSubmit={submit}
        initial={{ opacity: 0, y: 24 }}
        animate={{ opacity: 1, y: 0 }}
        className="w-full max-w-[600px] rounded-2xl border p-8 md:p-10"
        style={{ background: 'var(--card-bg)', borderColor: 'var(--border)', boxShadow: '0 0 70px var(--card-glow)' }}
      >
        <VZNAvatar size="md" className="mb-6" />
        <h1 className="text-3xl font-bold">Let&apos;s analyse your idea.</h1>
        ...
        <div className="mt-8 space-y-6">
          <label className="block">
            ...
            <textarea
              ...
              rows={5}
              maxLength={500}
              className="focus-ring mt-2 w-full resize-none rounded-xl border px-4 py-3 text-sm"
          ...
          <label className="block">
            ...
            <input
              ...
              className="focus-ring mt-2 w-full rounded-xl border px-4 py-3 text-sm"
              style={{ background: 'var(--bg-secondary)', borderColor: 'var(--border)', color: 'var(--text-primary)' }}
            />
          </label>
        ...
        <button type="submit" className="mt-7 flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--purple)] px-5 py-3.5 font-semibold text-white transition-all hover:brightness-110">
